...
import { deviceListHttp } from './device-list-http';
import { CreateDeviceRequest } from '../../device-create/models/create-device';
...
const INITIAL_COMMAND: DeviceListCommand = { action: StoreAction.Load };

@Injectable()
export class deviceListStore {
  private readonly _deviceListHttp = inject(deviceListHttp);

  private readonly _state = signal<DevicesState>(INITIAL_STATE);
  private readonly _command = signal<DeviceListCommand>(INITIAL_COMMAND);
  ...
  dispatch(action: StoreAction.Edit, payload: UpdateDeviceRequest & { id: string }): void;
  ...
  dispatch(action: StoreAction, payload?: CreateDeviceRequest | UpdateDevicePayload | DeleteDevicePayload | DeleteDevicesPayload): void {
    switch (action) {
      ...
      case StoreAction.Edit:
        ...
        this._command.set({ action: StoreAction.Edit, payload: payload as UpdateDevicePayload });
        return;
      case StoreAction.Delete:
  ...
  private loadDevices(): void {
    ...
    this._deviceListHttp.getAllDevices().subscribe({
      next: (devices) => {
        this._state.update((state) => ({
          ...state,
  ...
  private executeCreateDevice(device: CreateDeviceRequest): void {
    ...
    this._deviceListHttp.createDevice(device).subscribe({
      next: () => {
        this._state.update((state) => ({
          ...state,
          status: StoreStatus.Success,
  ...
  private executeUpdateDevice(device: UpdateDeviceRequest & { id: string }): void {
    ...
    this._deviceListHttp.updateDevice(device.id, deviceToEdit).subscribe({
      next: () => {
        this._state.update((state) => ({
          ...state,
          status: StoreStatus.Success,
  ...
  private deleteDevice(payload: DeleteDevicePayload): void {
    this._deviceListHttp.deleteDeviceById(payload.id).subscribe({
      next: () => {
        this._state.update((state) => ({
          ...state,
          status: StoreStatus.Success,
  ...
  private deleteDevices(payload: DeleteDevicesPayload): void {
    this._deviceListHttp.deleteAllSelectedDevices({ ids: payload.ids }).subscribe({
      next: () => {
        this._state.update((state) => ({
          ...state,
          status: StoreStatus.Success,
