...
import { deviceListStore } from './services/device-list-store';
...
export class deviceListComponent {
  private readonly _deviceListStore = inject(deviceListStore);
  protected readonly _storeStatus = StoreStatus;
  protected readonly _status = this._deviceListStore.status;
  ...
  readonly table = createAngularTable<Device>(() => ({
    data: this._deviceListStore.devices(),
  ...
  constructor() {
    effect(() => {
      if(this._deviceListStore.lastAction() === StoreAction.DeleteMultiple && this._deviceListStore.status() === StoreStatus.Success) {
  ...
  protected editDevice(device: Device): void {
    // TO DO : Add navigation to devices/:id/edit
  }

  private deleteDevice(device: Device): void {
    this._deviceListStore.dispatch(StoreAction.Delete, { id: device.id });
  }

  private deleteDevices(): void {
    const selectedDeviceIds = this.table.getSelectedRowModel().rows.map((row) => row.original.id);

    this._deviceListStore.dispatch(StoreAction.DeleteMultiple, { ids: selectedDeviceIds });
