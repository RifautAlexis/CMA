	...
	template: `
		...
		<ng-template #ActionDropDownMenu>
			<hlm-dropdown-menu>
				...
				<button hlmDropdownMenuItem (click)="deleteDevices()">Delete</button>
			</hlm-dropdown-menu>
		</ng-template>
	`,
})
export class ActionDropdownHeader {
	protected readonly deviceToDelete = output<void>();

	deleteDevices(): void {
		this.deviceToDelete.emit();
	}
}
